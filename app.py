...
data['Residence_type'] = st.selectbox("Residence Type", ['Urban', 'Rural'], key='residence_key')
data['Residence_type'] = 1 if data['Residence_type'] == 'Urban' else 0

data['avg_glucose_level'] = st.slider("Avg Glucose Level", 50.0, 250.0, 100.0, key='glucose_key')
data['bmi'] = st.slider("BMI", 10.0, 60.0, 25.0, key='bmi_key')

work_type_options = ['Private', 'Self-employed', 'children', 'Govt_job', 'Never_worked']
selected_work_type = st.selectbox("Work Type", work_type_options, key='work_type_select')
